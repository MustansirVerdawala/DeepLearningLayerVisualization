...
% Apply Leaky ReLU
relu_matrix = normalized_convolved;
relu_matrix(relu_matrix > 0) = alpha * relu_matrix(relu_matrix > 0);

relu_min = double(min(relu_matrix(:)));
relu_max = double(max(relu_matrix(:)));
relu_matrix = (double(relu_matrix) - relu_min) / (relu_max - relu_min);
relu_matrix(isnan(relu_matrix)) = 0;
% relu_matrix = relu_matrix - 0.5;
...
for row = 1:n_rows
    for col = 1:n_cols
        ...
        rectangle('Position', [right_grid_x + (col - 1) * cell_size, (row - 1) * cell_size, cell_size, cell_size], ...
                  'EdgeColor', 'w', 'LineWidth', 1, ...
                  'FaceColor', 1-[val_after val_after val_after]);
    end
end
